...
import { sideBarAtom, lastMessageAtom, userInfo } from "../../Variables";
...
const Navbar = () => {
    ...
    const [search, setSearch] = useState([
        {
            suggestion: ["10 students registered", "30 students registered"],
            type: "PMS",
            link: "/superadmin/pms",
        },
        {
            suggestion: ["36/40 Sem 6E", "36/40 Sem 6A"],
            type: "Attendance",
            link: "/superadmin",
        },
        {
            suggestion: ["Meeting on 12/12/2021", "Meeting on 12/12/2021"],
            type: "Calendar",
            link: "/superadmin/calendar",
        },
        {
            suggestion: ["chat 1", "chat 2"],
            type: "Chat",
    ...
    useEffect(() => {
        if (lastMessage !== null && lastMessage !== "" && lastMessage !== undefined) {
            ...
            if (jsonData !== null && jsonData !== undefined && jsonData !== "") {
                try {
                    let tempMsg = JSON.parse(jsonData.message).message;
                    setSearch((prev) => {
                        for (let i = 0; i < prev.length; i++) {
                            if (prev[i].type === "Chat") {
                                if (prev[i].suggestion.length > 5) {
                                    prev[i].suggestion = prev[i].suggestion.slice(prev[i].suggestion.length - 5, prev[i].suggestion.length);
                                }
                                prev[i].suggestion.push(tempMsg);
                            }
                        }
                    ...
                    <div className="flex items-center gap-2">
                        <div
                            ...
                            onClick={(event) => {
                                ...
                                newImg.src = process.env.REACT_APP_SERVER + "/api/main/avatarCreator?gender=" + userInfoAtom.gender + "&name=" + userInfoAtom.username;
                        ...
                        <p className="text-textColor-500 text-sm font-bold lg:block hidden">{userInfoAtom.first_name + " " + userInfoAtom.last_name}</p>
                    </div>
                    {/* Profile Model */}
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <div className="w-full bg-white pt-2.5">
                                {search.map((item, index) => (
                                    <div key={index}>
                                        {item.suggestion.map((message, index2) => (
                                            <div key={index2} className={"flex items-start gap-2 text-sm text-textColor-500 px-4 border-boderColor last-of-type:border-none cursor-pointer hover:bg-textColor-100 transition-all duration-300 " + (activeTab === item.type || activeTab === "All" ? "scale-100 py-2 border-b" : "py-0 scale-0 h-0 border-none")}>
                                                {item.type === "PMS" && (
                                                    <svg viewBox="0 0 24 24" fill="none" className="w-4 h-4">
                                                        <path d="M3 7c0-1.886 0-2.828.586-3.414C4.172 3 5.114 3 7 3h10c1.886 0 2.828 0 3.414.586C21 4.172 21 5.114 21 7v10c0 1.886 0 2.828-.586 3.414C19.828 21 18.886 21 17 21H7c-1.886 0-2.828 0-3.414-.586C3 19.828 3 18.886 3 17V7Z" stroke="currentColor" strokeWidth={2} />
                                                        <path fillRule="evenodd" clipRule="evenodd" d="M18 10h-5.343c-.818 0-1.226 0-1.594-.152-.368-.152-.657-.442-1.235-1.02l-.656-.656c-.578-.578-.868-.868-1.235-1.02C7.569 7 7.16 7 6.343 7H3v10c0 1.886 0 2.828.586 3.414C4.172 21 5.114 21 7 21h10c1.886 0 2.828 0 3.414-.586C21 19.828 21 18.886 21 17V7c0 .932 0 1.398-.152 1.765a2 2 0 0 1-1.083 1.083C19.398 10 18.932 10 18 10ZM7 15a1 1 0 1 0 0 2h8a1 1 0 1 0 0-2H7Z" fill="currentColor" />
