    ...
    useEffect(() => {
        if (lastMessage !== null && lastMessage !== "" && lastMessage !== undefined) {
            ...
            if (jsonData !== null && jsonData !== undefined && jsonData !== "") {
                try {
                    setMsgArr([...msgArr, JSON.parse(jsonData.message)]);
                    setTimeout(() => {
            ...
            <div className="w-[90%] lg:w-[95%] flex flex-nowrap mx-auto h-fit bg-white rounded-2xl shadowCustom mb-10 overflow-hidden">
                <div className="w-1/5 flex flex-col h-[80vh] border-r border-borderColor">
                    ...
                    <div className="h-full w-full flex flex-col">
                        ...
                        <div className="h-12 w-full px-3 pl-3 lg:pl-10 group flex flex-nowrap justify-between items-center cursor-pointer hover:bg-slate-100">
                            ...
                            <div className="flex flex-nowrap gap-2 items-center justify-end">
                                <p className="text-green-400 font-bold hidden lg:block">+2</p>
                                <p className="text-textColor-400 font-bold w-10 text-end hidden lg:block">56</p>
                            </div>
                        </div>
                        <div className="h-12 w-full px-3 pl-3 lg:pl-10 group flex flex-nowrap justify-between items-center cursor-pointer hover:bg-slate-100">
                            <p className="text-textColor-600 font-bold">All Assistant</p>
                            <div className="flex flex-nowrap gap-2 items-center justify-end">
                                <p className="text-green-400 font-bold hidden lg:block">+3</p>
                                <p className="text-textColor-400 font-bold w-10 text-end hidden lg:block">8</p>
                            </div>
                        </div>
                        <div className="h-12 w-full px-3 pl-3 lg:pl-10 group flex flex-nowrap justify-between items-center cursor-pointer hover:bg-slate-100 ">
                            <p className="text-textColor-600 font-bold">All Students</p>
                            <div className="flex flex-nowrap gap-2 items-center justify-end">
                                <p className="text-green-400 font-bold hidden lg:block">+10</p>
                                <p className="text-textColor-400 font-bold w-10 text-end hidden lg:block">485</p>
                            </div>
                        </div>
                        <div className="h-12 w-full px-3 pl-3 lg:pl-10 group flex flex-nowrap justify-between items-center cursor-pointer hover:bg-slate-100 ">
                            <p className="text-textColor-600 font-bold">CE Students</p>
                            <div className="flex flex-nowrap gap-2 items-center justify-end">
                                <p className="text-green-400 font-bold hidden lg:block">+11</p>
                                <p className="text-textColor-400 font-bold w-10 text-end hidden lg:block">400</p>
                            </div>
                        </div>
                        <div className="h-12 w-full px-3 pl-3 lg:pl-10 group flex flex-nowrap justify-between items-center cursor-pointer hover:bg-slate-100 ">
                            ...
                            <div className="flex flex-nowrap gap-2 items-center justify-end">
                                ...
                                <p className="text-textColor-400 font-bold w-10 text-end hidden lg:block">80</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div className="w-4/5 flex flex-col h-[80vh]">
                    ...
                    <div className="h-full w-full relative flex flex-col">
                        <div id="msgContainer" className="h-[calc(80vh-8.5rem)] overflow-y-scroll flex flex-col noScrollbar scroll-smooth">
                            {msgArr.map((msg, index) => {
                                return <Message key={index} name={msg.name} gender={msg.gender} role={msg.role} time={msg.time} message={msg.message} />;
                            })}
                        </div>
                            ...
                            <input
                                onKeyUp={(event) => {
                                    if (inputMsg.trim() !== "") {
                                        if (event.key === "Enter") {
                                            // do something
                                            fetch("/api/main/msgSender?msg=" + inputMsg)
                                                .then((response) => response.json())
                                                .then((result) => {
                                                    setMsgArr([...msgArr, JSON.parse(result.message)]);
                                                    setInputMsg("");
                                                })
                                                .catch((error) => {
                                                    console.log(error);
                                ...
                                className="w-full h-full p-2 pl-5 pr-14 bg-white text-textColor-600 text-sm outline-none"
                            />
                        </div>
                    </div>
                </div>
