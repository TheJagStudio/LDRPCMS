...
const AdminActions = ({ userData, setUserToDelete, setUserToView }) => {
    return (
...
const AdminUserImage = ({ name, gender }) => {
    return <img src={process.env.REACT_APP_SERVER + "/api/main/avatarCreator?gender=" + gender + "&name=" + name} alt="avatar" className="rounded-md border bg-primary-100 border-primary-800 w-10 h-10" />;
};

const AdminUsers = () => {
    const [selectAll, setSelectAll] = useState(false);
    const [activeUserType, setActiveUserType] = useState("All Users");
    const userTypes = ["All Users", "Faculty", "Lab Asst", "Student"];
        ...
        fetch(process.env.REACT_APP_SERVER + "/api/main/getUserDetails")
            ...
            .then((data) => {
                ...
                for (let i = 0; i < data.data.length; i++) {
                    if (data.data[i].role === activeUserType || activeUserType === "All Users") {
    ...
    return (
        <div className="flex flex-col w-full h-full">
            <UserviewModal data={userToView} />
            <UserdeleteModal data={userToDelete} users={users} setUsers={setUsers} activeUser={activeUser} setActiveUser={setActiveUser} activeUserType={activeUserType} />
            <div className="w-full">
                <div className="w-full h-full flex flex-wrap sm:gap-0 gap-5 items-center justify-between p-2 px-8 lg:p-9">
                    ...
                    <NavLink to="/admin/users/adduser" className="bg-primary-500 text-white flex flex-nowrap items-center px-3 py-2 gap-2 rounded-full w-fit h-12 purpleBtnShadow hover:-translate-y-1 transition-all group">
            ...
            <div className="w-full lg:w-[95%] mx-auto h-fit bg-white rounded-2xl shadow-md mb-10 overflow-hidden">
                ...
                <Grid
                    ...
                    data={activeUser.map((user) => [_(<AdminUserImage gender={user.gender} name={user.name} />), user.name, user.email, user.role, _(<AdminActions setUserToDelete={setUserToDelete} userData={user} setUserToView={setUserToView} />)])}
                    search={false}
                    pagination={{
                        limit: 25,
                        summary: true,
                ...
                {/* <Grid
                    ...
                    data={[
                        [_(<AdminUserImage gender="male" name="Jagrat Patel" />), "Jagrat Patel", "jagrat@patel.com", "HOD", _(<AdminActions />)],
                        [_(<AdminUserImage gender="male" name="Jaivin Barot" />), "Jaivin Barot", "jaivin@barot.com", "HOD", _(<AdminActions />)],
                        [_(<AdminUserImage gender="male" name="Shubham Bhogayata" />), "Shubham Bhogayata", "shubham@bhogayata.com", "HOD", _(<AdminActions />)],
                        [_(<AdminUserImage gender="male" name="Parth Padhiyar" />), "Parth Padhiyar", "parth@padhiyar.com", "HOD", _(<AdminActions />)],
                    ]}
                    search={false}
                    pagination={true}
                    sort={true}
